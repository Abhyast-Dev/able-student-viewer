...
function structureQuestion(q) {
  ...
  if (/^Fill in the blank:/i.test(text)) {
    return { ...q, type: "fill_blank", text: text.replace(/^Fill in the blank:\s*/i, "").trim() };
  }

  if (/^True or False:/i.test(text)) {
    return { ...q, type: "true_false", text: text.replace(/^True or False:\s*/i, "").trim() };
  }

  if (/^Direct Question:/i.test(text)) {
    return { ...q, type: "short_answer", text: text.replace(/^Direct Question:\s*/i, "").trim() };
  }
...
function normalizeAllQuestions() {
  ...
  if (assessment.type === "quiz") {
    ...
    (assessment.sections || []).forEach(s => {
      s.questions = (s.questions || []).map(structureQuestion);
    });
  }
}
...
function renderContinueLearning(attempts) {
  if (!attempts || !attempts.length) return "";

  const latest = attempts
    .slice()
    .sort((a, b) => new Date(b.lastAttemptAt || b.at) - new Date(a.lastAttemptAt || a.at))[0];

  if (!latest) return "";

  return `
    <div class="continue-box no-print">
      <h2>Continue Learning</h2>
      <p class="muted">
        ${esc(latest.title)}
        ${latest.type === "quiz" && latest.bestPercentage !== undefined ? ` · Best ${latest.bestPercentage}%` : ""}
      </p>
      ${latest.slug ? `
        <a class="btn secondary" href="./?id=${encodeURIComponent(latest.slug)}">
          ${latest.type === "quiz" ? "Retake" : "Open"}
        </a>
      ` : ""}
    </div>
  `;
...
function switchDashboardTab(tab) {
  ...
  if (!livePanel || !progressPanel || !tabLive || !tabProgress) return;
...
function applyDashboardFilters() {
  ...
  const list = $("#liveAssessments");
  if (!list) return;

  list.innerHTML = renderGroupedAssessments(filtered);
  renderMath();
}
  ...
  try {
    ...
    render(`
      <div class="error">
        ...
        Please check <code>assessments/${esc(id)}.json</code>.
      </div>
    `);
  }
}
...
function renderWrittenQuestion(q, index) {
  ...
  if (n.type === "mcq" && Array.isArray(n.options)) {
    return `
      <div class="question">
        <p><b>Q${index + 1}.</b> ${esc(n.text)} <span class="muted">[${esc(n.marks)} marks]</span></p>
        <div class="options-list">
          ${n.options.map((o, i) => `<div class="option-row"><b>${String.fromCharCode(65 + i)}.</b> ${esc(o)}</div>`).join("")}
        </div>
        ${n.responseLength ? `<p class="muted">Suggested response: ${esc(n.responseLength)}</p>` : ""}
      </div>
    `;
  }
  ...
  if (n.type === "true_false") {
    return `
      <div class="question">
        <p><b>Q${index + 1}.</b> ${formatQuestionText(n.text)} <span class="muted">[${esc(n.marks)} marks]</span></p>
  ...
  render(`
    <article>
      <div class="report-head">
        ...
        <div>
          <div class="muted">ABLE™ iAssess</div>
          <h1>${esc(a.title)}</h1>
          <div class="muted">
...
function renderQuizStart() {
  ...
  render(`
    <div class="quiz-card">
      ...
      ${allow ? `<p class="muted" style="margin-top:10px">Correct answers will be available after you submit the quiz.</p>` : ""}
...
function startQuiz() {
  const name = $("#studentName")?.value || quiz.studentName || "Student";
...
function quizInput(q, ans) {
  ...
  return (opts || []).map((o, i) => {
    ...
    return `<label class="option"><input type="radio" name="answer" value="${esc(val)}" ${ans === val || ans === o ? "checked" : ""}> ${esc(o)}</label>`;
  }).join("");
}
...
function correctAnswerDisplay(q) {
  ...
  if (q.type === "mcq" && Array.isArray(q.options)) {
    ...
    if (/^[A-Z]$/.test(u)) {
      ...
      if (q.options[idx]) return `${u}. ${q.options[idx]}`;
    }
  }
...
function studentAnswerDisplay(q, a) {
...
function isAnswerCorrect(q, a) {
  ...
  if (q.type === "mcq") {
    return String(a).trim().toUpperCase() === String(q.correctAnswer).trim().toUpperCase();
  }
...
function renderQuizResult(p) {
  const can = assessment.viewer?.showCorrectAnswersAfterSubmit !== false;

  render(`
    <div class="quiz-card" style="text-align:center">
      <h1>Quiz Submitted</h1>
      <p style="font-size:42px;font-weight:800">${p.score}/${p.total}</p>
      <p class="muted">${p.percentage}%</p>

      <div class="actions" style="justify-content:center">
        ${can ? `<button class="secondary" onclick="showAnswerReview()">Review Answers</button>` : ""}
    ...
    <div class="quiz-card">
      ...
      ${lastQuizResult.responses.map((r, i) => `
        <div class="question" style="border-left:4px solid ${r.isCorrect ? "#16a34a" : "#dc2626"};padding-left:14px">
          ...
          ${r.options?.length ? `
            <div class="options-list">
              ${r.options.map((o, idx) => `<div class="option-row"><b>${String.fromCharCode(65 + idx)}.</b> ${esc(o)}</div>`).join("")}
            </div>
          ` : ""}

          <p>Your answer: <b>${esc(r.answerDisplay || "Not answered")}</b></p>
          <p>Correct answer: <b>${esc(r.correctDisplay || r.correct || "Not provided")}</b></p>
